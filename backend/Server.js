...
app.post("/delist",async(req,res)=>{//this funciton is removing a vehcile from being listen for futer bookings
    ...
    const updateBookedLisiting  = await UserBook.updateOne({"vehicleId" : vehicleId},{$set : {"Delisted" : true}});
    if( delistResult.acknowledged){
        res.json({status: true,msg : "Successfuly Delisted "})
    }else{
        res.json({status: false,msg : "An Error Occcoured when deleting the listing "})
    }
})
