...
export default function MyListings({setCreateTabActive,setMyListingTabActive,userGlobalData}){
    ...
    const removeListingHandler = async(listing)=>{
        const removeLocalListing = ()=>{
            const updatedListings = myListing.filter(_listing => _listing._id !== listing._id);
            setMyListings(updatedListings);
        }
        removeLocalListing()
        try {
            const res = await fetch("http://localhost:8080/delist",{
                method : "POST",
                headers : {'Content-type' : 'application/json'},
                body : JSON.stringify({
            ...
            console.log("error in the removeListingHandler".error)
        }
    }
    useEffect(()=>{
        ...
        }
        {
            myListing.length > 0 && myListing.map(listing=>{
                ...
                <div className="moreOptionsContainer">
                    <button className="removeListingBtn" onClick={()=>removeListingHandler(listing)}><MdDelete size={19}/>Remove Listing</button>
                </div>
                }
            </div>
                )
